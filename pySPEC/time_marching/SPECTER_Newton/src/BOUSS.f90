...
!#define BOUSS_NEWT_ 'bouss/bouss_newt.f90'
# 45 "specter.fpp" 2
...
! Temporal data storage arrays
      ...
      COMPLEX(KIND=GP), ALLOCATABLE, DIMENSION (:,:,:) :: C18,C19,C20,C21
...
! Simulation, forcing and initial conditions parameters
      ...
      REAL(KIND=GP)    :: T_guess, T1, dt1, dt_aux, alpha
      INTEGER          :: step_evol, N1
      CHARACTER(len=100) :: odir_newt
...
! Namelists for the input files
      ...
      NAMELIST / sca_newt / T_guess,odir_newt,alpha,N1
# 251 "specter.fpp"
...
# 332 "specter.fpp"
      ALLOCATE( C18 (nz,ny,ista:iend), C19 (nz,ny,ista:iend), C20 (nz,ny,ista:iend), C21 (nz,ny,ista:iend) )
...
!     T_guess : Initial guess of the UPO period in time units. 
      ...
      CALL MPI_BCAST(odir_newt,100,MPI_CHARACTER,0,MPI_COMM_WORLD,ierr)
      CALL MPI_BCAST(alpha,1,GC_REAL,0,MPI_COMM_WORLD,ierr)
      CALL MPI_BCAST(N1,1,GC_REAL,0,MPI_COMM_WORLD,ierr)
...
! In Newton-Solver algorithm: performs evolution in T_guess period

            !INCLUDE 'bouss/bouss_deriv.f90'
            INCLUDE 'bouss/bouss_evol_T.f90'
            !INCLUDE 'bouss/bouss_sol_project.f90'
...
! with global quantities.
         ...
         IF ((timec.eq.cstep).and.(bench.eq.0)) THEN
            timec = 0
            INCLUDE 'bouss/bouss_global.f90'
         ENDIF
...
!$omp parallel do if (iend-ista.lt.nth) private (k)
         ...
         DO k = 1,nz
            ...
            INCLUDE 'bouss/bouss_rkstep1.f90'
...
! Evolves the system in time
         ...
         DO o = ord,1,-1
            INCLUDE 'bouss/bouss_rkstep2.f90'
         END DO
            timet = timet+1
            timec = timec+1
