...
!$    CALL dfftw_plan_with_nthreads(nth)
      ...
      IF (fftdir .eq. -1) THEN
      CALL dfftw_plan_many_dft_r2c(plan%planrxy,2,(/n(1),n(2)/),          &
                         kend-ksta+1,plan%rarr,                               &
                         (/n(1),n(2)*(kend-ksta+1)/),1,n(1)*n(2),             &
                         plan%carr,(/n(1)/2+1,n(2)*(kend-ksta+1)/),1,         &
                         (n(1)/2+1)*n(2),flags)
      CALL dfftw_plan_many_dft(plan%plancz,1,n(3),n(2)*(iend-ista+1),     &
                         plan%ccarr,(iend-ista+1)*n(2)*n(3),1,n(3),           &
                         plan%ccarr,(iend-ista+1)*n(2)*n(3),1,n(3),           &
                         -1,flags)

      ELSEIF (fftdir .eq. 1) THEN
      CALL dfftw_plan_many_dft_c2r(plan%planrxy,2,(/n(1),n(2)/),          &
      ...
      IF (fftdir .eq. -1) THEN
      ! Create X -> YZ plan
      CALL dfftw_plan_many_dft_r2c(plan%planrx,1,n(1),                    &
                         n(2)*(kend-ksta+1),plan%rarr,n(1)*n(2)*(kend-ksta+1),&
                         1,n(1),plan%carr,(n(1)/2+1)*n(2)*(kend-ksta+1),1,    &
                         (n(1)/2+1),flags)
      CALL dfftw_plan_many_dft(plan%plancyz,2,(/n(3),n(2)/),(iend-ista+1),&
                         plan%ccarr,(/n(3),n(2)*(iend-ista+1)/),1,n(3)*n(2),  &
                         plan%ccarr,(/n(3),n(2)*(iend-ista+1)/),1,n(3)*n(2),  &
                         -1,flags)
      ELSEIF (fftdir .eq. 0) THEN
      CALL dfftw_plan_many_dft_c2r(plan%planrx,1,n(1),n(2)*(kend-ksta+1), &
...
!-----------------------------------------------------------------
      ...
      CALL dfftw_destroy_plan(plan%planrxy)
      CALL dfftw_destroy_plan(plan%planrx)
      CALL dfftw_destroy_plan(plan%plancyz)
      CALL dfftw_destroy_plan(plan%plancz)
...
!
      CALL GTStart(hfft)
      CALL dfftw_execute_dft_r2c(plan%planrc%planrxy,in,plan%planrc%carr)
...
!
      CALL GTStart(hfft)
      CALL dfftw_execute_dft(plan%planrc%plancyz,inout,inout)
      CALL GTStop(hfft); ffttime = ffttime + GTGetTime(hfft)
...
!
      CALL GTStart(hfft)
      CALL dfftw_execute_dft_r2c(plan%planrc%planrx,in,plan%planrc%carr)
      CALL GTStop(hfft); ffttime = ffttime + GTGetTime(hfft); 
...
!
      CALL GTStart(hfft)
      CALL dfftw_execute_dft(plan%planrc%plancz,inout,inout)
      CALL GTStop(hfft); ffttime = ffttime + GTGetTime(hfft)
...
!
      CALL GTStart(hfft)
      CALL dfftw_execute_dft_c2r(plan%plancr%planrxy,plan%plancr%carr,out)
      CALL GTStop(hfft); ffttime = ffttime + GTGetTime(hfft)
...
! 1D FFT in each node using the FFTW library
      CALL GTStart(htot)
      CALL GTStart(hfft)
      CALL dfftw_execute_dft(plan%plancr%plancyz,inout,inout)
      CALL GTStop(hfft); ffttime = ffttime + GTGetTime(hfft)
      CALL GTStop(htot); tottime = tottime + GTGetTime(htot)
...
!
      CALL GTStart(hfft)
      CALL dfftw_execute_dft_c2r(plan%plancr%planrx,plan%plancr%carr,out)
      CALL GTStop(hfft); ffttime = ffttime + GTGetTime(hfft)
...
! 1D FFT in each node using the FFTW library
      CALL GTStart(htot)
      CALL GTStart(hfft)
      CALL dfftw_execute_dft(plan%plancr%plancz,inout,inout)
      CALL GTStop(hfft); ffttime = ffttime + GTGetTime(hfft)
      CALL GTStop(htot); tottime = tottime + GTGetTime(htot)
