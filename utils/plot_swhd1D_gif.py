...
U = [np.load(f'{pm.out_path}/uu_{tidx:04}.npy') for tidx in range(0, steps, pm.ostep)]
H = [np.load(f'{pm.out_path}/hh_{tidx:04}.npy') for tidx in range(0, steps, pm.ostep)]

# plot gifs for u and h and save to out_path
plot_time_lapse(U ,steps, Lx = pm.Lx , Nx = pm.Nx,
                name = f'{pm.out_path}/u.gif',
                    color = 'blue' ,
                    label = 'u',
                    interval = 200 ,
                    fps = 5 )
plot_time_lapse(H ,steps, Lx = pm.Lx , Nx = pm.Nx,
                name = f'{pm.out_path}/h.gif',
                    color = 'green' ,
                    label = 'h',
                    interval = 200 ,
                    fps = 5 )
